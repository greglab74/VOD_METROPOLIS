...
{
   $reponse5 = $dbh->query('SELECT ID_films, affiche FROM films NATURAL JOIN A NATURAL JOIN genre WHERE ID_genre = "'.$genre.'"');
}
else {
	if (isset($_POST['realisateur']))
	{
	   $reponse5 = $dbh->query('SELECT ID_films, affiche FROM films NATURAL JOIN realise NATURAL JOIN realisateurs WHERE ID_realisateurs = "'.$realisateur.'"');
	}
	else {
		if (isset($_POST['acteur']))
		{
			$reponse5 = $dbh->query('SELECT ID_films, affiche FROM films NATURAL JOIN joue NATURAL JOIN acteurs WHERE ID_acteurs = "'.$acteur.'"');

		}
		else {
			$reponse5 = $dbh->query('SELECT ID_films, affiche FROM films WHERE affiche = 1');
			echo 'la recherche n a donné aucun résultat';
		}
	}
}
//SELECT * FROM film WHERE titrefr LIKE '$like' ORDER BY IDfilm DESC
		?>
